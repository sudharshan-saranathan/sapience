...
schemaViewer::schemaViewer(QWidget *parent) :
	...
	//  struct _attr_:
	attr{QRect(0, 0, SCENE_XS, SCENE_YS), ZOOM_INIT, ZOOM_MAX, ZOOM_MIN},
	//  struct _canvas_:
	canvas{new schemaCanvas(attr.bound, this)}
...
void schemaViewer::mousePressEvent(QMouseEvent *event) {
	//  qDebug() << "-------------------------------------------------";
	//  qDebug() << "schemaViewer::mousePressEvent()";
	QGraphicsView::mousePressEvent(event);
}

void schemaViewer::mouseReleaseEvent(QMouseEvent *event) {
	/*  Resolve event-handlers of schemaCanvas and its children first   */
	QGraphicsView::mouseReleaseEvent(event);

	//  printf() debugging:
	//  qDebug() << "schemaViewer::mouseReleaseEvent()";
}
